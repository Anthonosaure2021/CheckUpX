...
$Bcam.Add_Click({
    ...
    if ($cameraDevice) {
        ...
        $camPath = "shell:appsfolder\microsoft.windowscamera_8wekyb3d8bbwe!app"

        if (Test-Path $camPath) {
            $cam = explorer $camPath
            ...
            # Gérer le cas où l'application de la caméra n'est pas trouvée
            [System.Windows.Forms.MessageBox]::Show("L'application de la caméra n'est pas disponible sur cet ordinateur.", "Erreur")
        }
    } else {
